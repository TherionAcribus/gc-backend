...
_DEG_WORDS = {"deg", "degree", "degrees"}
_MIN_WORDS = {"min", "minute", "minutes"}
_DEC_SEP_WORDS = {"comma", "point", "dot"}
_CONNECTORS = {"and", "of", "the", "a"}
...
def _parse_en_int(tokens: List[str], i: int) -> Tuple[Optional[int], int]:
    if i >= len(tokens):
        return None, i
...
def _parse_digit_sequence(tokens: List[str], i: int, max_len: int) -> Tuple[str, int]:
    j = i
    out: List[str] = []
    while j < len(tokens) and len(out) < max_len:
        t = tokens[j]
        if t in _DIGIT_WORDS:
            out.append(_DIGIT_WORDS[t])
            j += 1
            continue
...
def _parse_minutes(tokens: List[str], i: int) -> Tuple[Optional[float], int, Dict[str, Any]]:
    ...
    whole, j = _parse_en_int(tokens, i)
    if whole is None:
        return None, i, meta
...
def _find_candidates(tokens: List[str], max_candidates: int) -> List[Dict[str, Any]]:
    ...
    for i in range(len(tokens)):
        ...
        lat_deg, j2 = _parse_en_int(tokens, j)
        if lat_deg is None:
            continue
        ...
        lon_deg, j2 = _parse_en_int(tokens, j)
        if lon_deg is None:
            continue
        j = _skip_connectors(tokens, j2)
...
class WrittenCoordsEnPlugin:
    def __init__(self):
        self.name = "written_coords_en"
        self.description = "Recognize GPS coordinates written in English"
    ...
    def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "status": "success",
            "summary": f"{len(candidates)} EN candidates",
            "results": candidates,
            "plugin_info": {
                "name": self.name,
                "version": "1.0.0",
